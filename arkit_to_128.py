...
class ArkitJsonTo128Converter:
    ...
    def extract_optional_pose9(self, value):
        """解析可选 tracker pose。

        支持两种输入:
          - 3 维位置: [x, y, z]，rot6d 使用单位旋转占位
          - 4x4 齐次矩阵: 左上 3x3 为旋转，第 4 列前三个为位置
        兼容 flat 16 数组，按 4x4 reshape。
        """
        arr = np.asarray(value, dtype=np.float32)
        if arr.shape == (3,):
            return np.concatenate([arr, IDENTITY_ROT6D]).astype(np.float32)
        if arr.shape == (4, 4):
            return np.concatenate([
                self.extract_position(arr),
                self.extract_6d_rotation(arr[:3, :3])
            ]).astype(np.float32)
        if arr.size == 16:
            mat = arr.reshape(4, 4)
            return np.concatenate([
                self.extract_position(mat),
                self.extract_6d_rotation(mat[:3, :3])
            ]).astype(np.float32)
        raise ValueError(f"Expected tracker pose as xyz or 4x4 matrix, got shape {arr.shape}")
    ...
    def load_json_frames(self, json_path):
        """读取 JSON，兼容直接帧列表和带 metadata + frames 的格式。"""
        ...
        if isinstance(payload, dict) and isinstance(payload.get('frames'), list):
            ...
            return payload['frames'], metadata
        raise ValueError(
            f"Unsupported JSON format in {json_path}: expected a frame list or a dict with a frames list"
        )
    ...
    def extract_video_frames(self, mp4_path, target_frames, max_size_mb=5):
        ...
        if len(frames) > target_frames:
            ...
            print(f"  警告: 视频帧数 {len(frames)} 少于动作帧数 {target_frames}，用最后一帧补齐")
    ...
    def convert_single(self, json_path, mp4_path=None):
        """转换单个 JSON 文件"""
        frames, metadata = self.load_json_frames(json_path)
        ...
        for frame in frames:
            ...
            # Optional Vive tracker poses:
            # old data has xyz only, new process_episode.py data may provide 4x4 matrices.
            waist_valid = bool(frame.get('waist_valid', 'waist_V' in frame))
            if waist_valid and 'waist_V' in frame:
                action[89:98] = self.extract_optional_pose9(frame['waist_V'])

            head_tracker_valid = bool(frame.get('head_tracker_valid', 'head_tracker_V' in frame))
            if head_tracker_valid and 'head_tracker_V' in frame:
                action[58:67] = self.extract_optional_pose9(frame['head_tracker_V'])

            waist_valid_flags.append(waist_valid)
            head_tracker_valid_flags.append(head_tracker_valid)
        ...
        with h5py.File(output_path, 'w') as f:
            ...
            if waist_valid_flags:
                f.create_dataset('waist_valid', data=np.asarray(waist_valid_flags, dtype=np.bool_))
            if head_tracker_valid_flags:
                f.create_dataset('head_tracker_valid', data=np.asarray(head_tracker_valid_flags, dtype=np.bool_))
            ...
            if metadata:
                for key in ('episode', 'note', 'tracker_file', 'time_offset_sec'):
                    if key in metadata:
    ...
    def convert_batch(self, file_list=None):
        ...
        for json_path in tqdm(json_files, desc="转换进度"):
            # 从 wholebody-N*.json 提取编号 N，兼容 wholebody-N_unified_V.json
            json_name = json_path.stem  # wholebody-1
            episode_name = json_name
            for suffix in ('_unified_V', '_unified', '_V'):
                if episode_name.endswith(suffix):
                    episode_name = episode_name[:-len(suffix)]

            mp4_bases = [episode_name]
            if json_name.startswith('wholebody-'):
                mp4_bases.append('wb-' + episode_name.replace('wholebody-', ''))
            if json_name != episode_name:
                mp4_bases.append(json_name)
            
            # 尝试多种可能的文件名（处理可能的前导空格）
            mp4_path = None
            for mp4_base in dict.fromkeys(mp4_bases):
            ...
            if mp4_path is None:
                print(f"  ⚠️ 未找到视频文件: {episode_name}.MP4")
