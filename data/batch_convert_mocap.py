...
def convert_wholebody_json_to_processed(
    ...
    *,
    ...
    description: str = "",
    embodiment: str = "human_mocap_annotated",
) -> None:
    ...
    dst_hdf5_path.parent.mkdir(parents=True, exist_ok=True)
    with h5py.File(dst_hdf5_path, "w") as f_out:
        ...
        f_out.attrs["embodiment"] = embodiment
        f_out.attrs["description"] = description
...
def _load_or_init_descriptions(pkl_path: Path) -> dict:
    if pkl_path.exists():
...
def batch_process_hdf5_range(
    ...
    *,
    ...
    pkl_path = output_base_dir / f"{task_name}.pkl"
    descriptions = _load_or_init_descriptions(pkl_path)
    processed_count = 0
    for i in tqdm(range(start_idx, end_idx + 1)):
        ...
        try:
            convert_mocap_to_processed(
                ...
                write_left_right=write_left_right,
    ...
    print(f"Saved language embeddings to {pkl_path}")
...
if __name__ == "__main__":
    ...
    p.add_argument("--output-base-dir", type=str, default=None)
    p.add_argument("--task-name", type=str, default=None)
    p.add_argument("--format", type=str, default="auto", choices=["auto", "hdf5", "wholebody_json"])
    ...
    p.add_argument("--cam-to-sim", type=str, default=None)
    ...
    p.add_argument("--no-head-as-origin", action="store_true")
    p.add_argument("--kpts-frame", type=str, default=None, choices=["wrist", "head", "world"])
    p.add_argument("--no-shift-action", action="store_true")
    p.add_argument("--write-left-right", action="store_true")
    p.add_argument("--compress-images", action="store_true")
    p.add_argument("--sbs", action="store_true")
    p.add_argument("--for-mujoco", action="store_true")
    ...
    if fmt == "hdf5":
        for_mujoco = bool(args.for_mujoco)
        cam_to_sim = ("default" if for_mujoco and args.cam_to_sim is None else args.cam_to_sim)
        rel_frame = (args.rel_frame if args.rel_frame is not None else ("head_each" if for_mujoco else "none"))
        still_head = bool(args.still_head or for_mujoco)
        head_as_origin = bool(args.head_as_origin or (for_mujoco and not args.no_head_as_origin))
        shift_action = not bool(args.no_shift_action or for_mujoco)
        state_from_prev_action = bool(args.state_from_prev_action)
        write_left_right = bool(args.write_left_right or for_mujoco)
        compress_images = bool(args.compress_images or for_mujoco)
        zero_head_translation = bool((args.zero_head_translation or for_mujoco) and not args.no_zero_head_translation)
