...
def evaluate(decoded: dict, args: argparse.Namespace) -> dict:
    ...
    left_thumb_z_lower_margin = np.max(left_non_thumb[:, :, 2], axis=1) - left_thumb[:, 2]
    right_thumb_z_lower_margin = np.max(right_non_thumb[:, :, 2], axis=1) - right_thumb[:, 2]
    ...
    checks = [
        ...
        check_per_frame(
            "left_thumb_tip_global_z_is_less_than_highest_other_left_fingertip",
            left_thumb_z_lower_margin,
            left_thumb_z_lower_margin > args.thumb_z_margin,
            args.min_pass_rate,
        ),
        axis_relation_check(
        ...
        check_per_frame(
            "right_thumb_tip_global_z_is_less_than_highest_other_right_fingertip",
            right_thumb_z_lower_margin,
            right_thumb_z_lower_margin > args.thumb_z_margin,
            args.min_pass_rate,
        ),
    ]
    ...
    return {
        "requirements": [
            ...
            "Left thumb tip global Z coordinate must be less than the highest other left fingertip.",
            ...
            "Right thumb tip global Z coordinate must be less than the highest other right fingertip.",
        ],
        "checks": checks,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate Inspire-real hand orientation constraints.")
