...
mkdir -p logs/bottle_mix "$CKPT_ROOT"
...
RUNS=(
  "0|c30_vjepa_w0   |30hz|$CFG_DIR/act_input_vjepa.yaml              |$DINO_COMMON --future_dino_weight 0.0"
  "1|c30_ab4        |30hz|$CFG_DIR/act_with_future_dino.yaml         |$DINO_COMMON --future_dino_weight 1.0"
  "2|c30_ab4_nostate|30hz|$CFG_DIR/act_with_future_dino_nostate.yaml |$DINO_COMMON --future_dino_weight 1.0 --zero_state_dims 98:128"
  "3|c30_flow       |30hz|$CFG_DIR/act_with_future_flow.yaml         |--use_future_flow_head --future_flow_weight 1.0 --future_flow_dir FLOWDIR"
  "4|c10_vjepa_w0   |10hz|$CFG_DIR/act_input_vjepa.yaml              |$DINO_COMMON --future_dino_weight 0.0"
  "5|c10_ab4        |10hz|$CFG_DIR/act_with_future_dino.yaml         |$DINO_COMMON --future_dino_weight 1.0"
  "6|c10_ab4_nostate|10hz|$CFG_DIR/act_with_future_dino_nostate.yaml |$DINO_COMMON --future_dino_weight 1.0 --zero_state_dims 98:128"
  "7|c10_flow       |10hz|$CFG_DIR/act_with_future_flow.yaml         |--use_future_flow_head --future_flow_weight 1.0 --future_flow_dir FLOWDIR"
)
...
for spec in "${RUNS[@]}"; do
  ...
  MIXED="/home/aigc/human_policy/mix_bottle_${tag}.json"
  if [ ! -s "$MIXED" ]; then echo "缺少数据配置: $MIXED"; exit 1; fi

  if [[ "$extra" == *FLOWDIR* ]]; then
    # flow 目标必须齐：训练时缺一条目标 = 那条 episode 的 world loss 静默变 0
    want=0
    for d in bottle_human_train bottle_human_val "mix_robot_${tag}_train" "mix_robot_${tag}_val"; do
...
dirs = ['bottle_human_train', 'bottle_human_val',
        f'mix_robot_{tag}_train', f'mix_robot_{tag}_val']
n = 0
for d in dirs:
...
PYEOF
  ...
  log="logs/bottle_mix/${exptid}.log"
