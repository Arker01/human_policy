...
learning_rate=1e-5
chunk_size=100
expt_id="train_episodes_45_95_dinov2"

# 数据和模型配置
model_cfg_path="hdt/configs/models/act_dinov2.yaml"
...
# ==========================================
cat > /tmp/episodes_45_95_dinov2_train.json << 'EOF'
{
  "train": [
    {
...
CUDA_VISIBLE_DEVICES=3 python hdt/main.py \
    --batch_size $batch_size \
    --num_epochs 50000 \
    --lr $learning_rate \
    --chunk_size $chunk_size \
    --exptid "$expt_id" \
    --dataset_json_path /tmp/episodes_45_95_dinov2_train.json \
